...
int numberOfComponents( std::vector<float> eigenValues)
{
    ...
    for(eit = eigenValues.begin(), eend = eigenValues.end() ; eit != eend ; eit ++)
    {
        ...
        std::cout<<"cumulative variance"<<cumulativeVariance<<std::endl;
...
int numberOfComponentsEigen( Eigen::JacobiSVD<Eigen::MatrixXd> svd)
{
    ...
    {
        sumEigenValues += svd.singularValues()[i];
        std::cout<<"Sing val "<<svd.singularValues()[i];
    }
    std::cout<<"Sum sing value"<<sumEigenValues<<std::endl;
    int nbCompo = 0;
    float cumulativeVariance = 0 ;
    for(int i = 0 ; i < svd.singularValues().rows() ; i++)
    {
        nbCompo ++;
        cumulativeVariance  +=  (svd.singularValues()[i] / sumEigenValues) ;
        std::cout<<"cumulative variance"<<cumulativeVariance<<std::endl;
        if(cumulativeVariance > 0.90)
        {
            return nbCompo;
...
int main ( int argc, char *argv[] )
{
  ...
  //  std::cout<<dataset<<std::endl;
    ...
    std::cout<<"SIZEEE SEARCH "<<dataset.rows()<<std::endl;
...
//    }
    ...
    std::vector <float> singVector;
    for(int i = 0 ; i< svd.singularValues().rows() ; i++){
           singVector.push_back(svd.singularValues()[i]);
    }
    int nbCompo2 = numberOfComponents(singVector);
     std::cout<<"Number of components 2 : "<<nbCompo2<<std::endl;
    //Search number of components to have 90% of cumulative variance
    int nbCompo;
    nbCompo = numberOfComponentsEigen(svd);
    std::cout<<"length singular value"<<svd.singularValues().rows()<<svd.singularValues().cols()<<std::endl;
    std::cout<<"Number of components : "<<nbCompo<<std::endl;
    ...
    for (int k = -10 ; k <= 10 ; k++ )
    {
        ...
        Eigen::MatrixXd reconstruction = meanDataset.transpose();
        for(int j=0 ; j < nbCompo ; j++)
        {
            float A = K *sqrt(svd.singularValues()[j]);

        std::cout<<"Scalar"<<A<<std::endl;
        Eigen::MatrixXd  vector =  svd.matrixU().col(j);
